...
Consequently, the image $I(\cdot,\cdot)$ is constructed by placing the sample points according to
...
\noindent  where $\mathbf{p}$ is a pixel from the image $I(\cdot,\cdot)$,  $\theta$ is the angle bin with $ \theta \in \{0, 45, 90, 135, 180, 225, 270, 315\} $,  $ |J(\mathbf{p})| $ is the norm of the gradient vector in the pixel $\mathbf{p}$ and it is computed using finite differences and $\angle J(\mathbf{p}) $ is the angle of the gradient vector.  The scalar $ w_\mathrm{ang}(\cdot) $  and vector $ w_{ij}(\cdot) $ functions are linear interpolations used by~\cite{Lowe2004} and \cite{Vedaldi2010} to provide a weighting contribution to eight adjacent bins.  They are calculated as  

\begin{equation}
 w_{ij}(\mathbf{v}) = w( v_x - x_i ) w( v_y - y_i ) / i,j \in \{0,1,2,3\} 
\label{eq:ij}
\end{equation}

\begin{equation}
 w_\mathrm{ang}(\alpha) = \sum_{r} w( \frac{8\alpha}{2\pi} + 8r)
\label{eq:wang}
\end{equation}

\noindent where $x_i$ and $y_i$ are the spatial bin centers located in $ x_i,y_i \in \{-\frac{3}{2},-\frac{1}{2},\frac{1}{2},\frac{3}{2}\} $, $\mathbf{v} = ( v_x, v_y ) $ is a dummy vector variable and $\alpha$ a dummy scalar variable.  On the other hand, $r$ is an integer that can vary freely which allows the argument $\alpha$ to be unconstrained in terms of its values in radians. The interpolating function $w(\cdot)$ is defined as:
...
During the calibration phase, two descriptors are extracted from averaged signal segments, corresponding to the chosen letter.  These descriptors are the P300 templates, grouped in a template set called $ T $.  

This set is constructed using the steps described in Section \ref{Average} and the steps A and B of the P300 ERP extraction process.

\paragraph{Letter identification}

Once the calibration process has been conducted and the template $T$ is constructed, the aim is to identify the selected letter from the matrix, based on the descriptors obtained in Section \ref{Average} and the steps A and B of the previously described P300 ERP extraction process.
...
\noindent where $N_T(d^l_u),\;l\in\{row,{col}\}$  is defined as $N_T(d^l_u) = \{d \in T / $  is the k-nearest neighbor of $ d^l_u \}$.  This set is obtained by sorting all the elements in $T$ based on euclidean distances $\mathcal{L}_2$ between them and $d^l_u$, choosing the $k$ with smaller values. This procedure is based on the k-NBNN  algorithm~\citep{Boiman2008}.
...
\begin{tabular}{ccccc}
\toprule
\textbf{Participant}	&  \textbf{BPC}	& \textbf{Character Recognition Rates}\\
\midrule
1     &     Cz   &   $35\%$  & $35\%$ &     $40\%$   \\
2     &     Fz   &   $85\%$  & $35\%$  &     $40\%$  \\
3     &     Cz   &   $25\%$  & $35\%$  &     $40\%$  \\
