...
\label{averaging}
\end{equation}  
for $1 \leq n \leq n_{max}$ and for the channels $1 \leq c \leq C$.  This provide an averaged signal $x^l(n,c)$ for the twelve locations.
\end{enumerate}
\end{itemize}
...
\begin{equation}
\tilde{x}^l(n,c) = \left \lfloor{ \gamma \cdot \frac{( x^l(n,c) - \bar{x}^l(c)  )}{ \hat{\sigma}^l(c) } }\right \rfloor, \; 1 \leq n \leq n_{max}, 1 \leq c \leq C
...
\noindent where $ (z_1,z_2) \in \mathbb{N} \times \mathbb{N}$ iterate over the width (based on the length of the signal segment) and height (based on the peak-to-peak amplitude) of the newly created image,  $1 \leq n \leq n_{max}$ and $1 \leq c \leq C$.  The value $z^l(c)$ is the location on the image where the signal's zero value has to be located in order to fit the entire signal within the image for each channel c:
...
\noindent where the minimization and maximization are carried out for $n$ varying between ${1 \leq n\leq n_{max}}$, and $ \lfloor \rfloor $ denote the rounding to the smaller nearest integer of the number.
...
%Rodrigo, acá pondría un punto y aparte. Aclararía que la intención de las tablas 2 y 4 son comparar los resultados de HIST con los resultados obtenidos por los autores que generaron loa datos de ALS pero como estos no informan el desempeño para clasificar las letras, entonces vos lo implementaste.No se si me saldrá bien en inglés pero lo que sigue lo pongo como idea:On the other hand, we shown Table 2 and 4 in order to compare the performance of our method against the method proposed by Riccio et al. Since the work of Riccio et al. does not measure the rate of well identification letter (they just report the rate of well classified P300 events) we completed the proccess  using SWLDA classificaion method (suggestied by  the article Riccio et al. 2013).

In Tables ~\ref{tab:resultsals} and ~\ref{tab:resultsown} results for character recognition rates using single channel signals with the SVM classifier and using a feature based on Permutation Entropy and classified by SVM (PE+SVM) are also shown.  The PE algorithm, which is also based on a time-domain description of the waveform, was implemented according to \cite{Unakafova2013} and its parameters were adjusted as stated by \cite{Zanin2012}, with an \textit{order} of $2$ and a \textit{sliding window} of size $10$.   The SVM classifier, on the other hand,  was configured to use a linear kernel.  

Moreover, Tables \ref{tab:resultsalsswlda} and \ref{tab:resultsownswlda} are presented in order to compare the performance of the Histogram of Gradient Orientations (HIST) method against a feature formed by concatenating all the channels \citep{Krusienski2006} and the classification algorithm SWLDA, the methodology proposed by the ALS dataset's publisher. Since authors \cite{Riccio2013} did not report the  Character Recognition Rate obtained for this dataset, we replicate their procedure and include the performance obtained with the SWLDA algorithm at letter identification. The obtained performance for both datasets is improved in 6 out 8 subjects.
...
It was verified that this method has an improved performance at letter identification than other methods that process the signals on a channel by channel strategy, and it even has a slightly improved performance compared to other methods like SWLDA, which uses a multichannel feature.
...
\begin{table}[htb]
\caption{Character recognition rates for the public dataset of ALS patients using the Histogram of Gradient (HIST) and performance rates obtained by the SWLDA with a multichannel concatenated feature.}
\centering
%% \tablesize{} %% You can specify the fontsize here, e.g.  \tablesize{\footnotesize}. If commented out \small will be used.
...
%\cline{1-5} \\
\textbf{Participant}	&  \textbf{BPC}	&  HIST & SWLDA  \\
\midrule
1     &     Cz   &   $35\%$  & $45\%$  \\
2     &     Fz   &   $85\%$  & $30\%$    \\
3     &     Cz   &   $25\%$  & $65\%$  \\
...
\caption{P300 template patches for subjects 8 (A) and 3 (B) of the ALS Dataset.  Downward deflection is positive polarity. }
