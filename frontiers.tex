...
In Tables~\ref{tab:resultsals} and~\ref{tab:resultsown} results for character recognition rates using single channel signals with the SVM~\citep{Scholkopf2001} classifier are also shown.    The SVM classifier, on the other hand,  was configured to use a linear kernel.  
...
\begin{table}[htb]
\caption{Character recognition rates for the public dataset of ALS patients using the Histogram of Gradient (HIST) and performance rates obtained by the SWLDA algorithm and the SVM algorithm, with a multichannel concatenated feature.}
\centering
%% \tablesize{} %% You can specify the fontsize here, e.g.  \tablesize{\footnotesize}. If commented out \small will be used.
...
%\cline{1-5} \\
\textbf{Participant}	&  \textbf{BPC}	&  HIST & SWLDA & SVM \\
\midrule
1     &     Cz   &   $35\%$  & $45\%$  & $40\%$\\
2     &     Fz   &   $85\%$  & $30\%$   & $50\%$   \\
3     &     Cz   &   $25\%$  & $65\%$ & $55\%$   \\
