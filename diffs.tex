...
%DIF ADD frontiers.tex       Tue Mar 12 19:21:12 2019
...
\noindent where $x_i$ and $y_i$ are the spatial bin centers located in $ x_i,y_j \in \{-\frac{3}{2},-\frac{1}{2},\frac{1}{2},\frac{3}{2}\} $\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{.  The function parameter }\DIFaddend $\mathbf{v} = ( v_x, v_y ) $ is a vector variable and $\alpha$ a scalar variable.  \DIFaddbegin \DIFadd{Vector $\mathbf{v}$ holds pixel $\mathbf{p}$  coordinates on the image in normalized bin units.   }\DIFaddend On the other hand, $r$ is an integer that can vary freely \DIFdelbegin \DIFdel{between $ \left[ -1,1 \right]$ which }\DIFdelend \DIFaddbegin \DIFadd{in the set $\{ -1, 0, 1 \} $ and }\DIFaddend allows the argument $\alpha$ to be unconstrained in terms of its values in radians. The interpolating function $w(\cdot)$ is defined as $ w(z) = \max(0,|z|-1)$.
...
is a classification algorithm that allows the categorization of an image to one class by comparing the set of extracted descriptors to those which are more similar from template dictionaries.  This work propose an adapted  discriminative~\mbox{%DIFAUXCMD
\citep{WolpawJonathanR2012} }%DIFAUXCMD
semi-supervised unary classification scheme to identify the selected letter in the P300-Based BCI Speller, based on the features provided by the calculated descriptors.  
}
...
Computer Vision libraries on MATLAB V2014a }\DIFdelend \DIFaddbegin \DIFadd{on MATLAB V2017a }\DIFaddend (Mathworks Inc., Natick, MA, USA). \DIFaddbegin \DIFadd{The algorithm described in~\ref{SIFT} is written on a modified version of the  VLFeat~\mbox{%DIFAUXCMD
\citep{Vedaldi2010} }%DIFAUXCMD
