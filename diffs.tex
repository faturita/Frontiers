...
%DIF ADD frontiers.tex       Wed Mar 13 15:55:11 2019
...
which is composed of two parts, the SIFT Detector and the SIFT Descriptor.  The former is the procedure to identify relevant areas of an image whereas the latter is the procedure to describe and characterize a region of an image (i.e. patch) calculating an histogram of the gradient orientations.  In order to characterize EEG signal waveforms, this work proposes an alternative Histogram of Gradient Orientations algorithm.  We stripped away all the details which are not used and refined the procedure to enable the effective capturing of the waveform shape.
}
...
\noindent where $x_i$ and $y_i$ are the spatial bin centers located in $ x_i,y_j \in \{-\frac{3}{2},-\frac{1}{2},\frac{1}{2},\frac{3}{2}\} $ \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and the interpolating function $w(\cdot)$ is defined as $ w(z) = \max(0,|z|-1)$. The function parameter }\DIFaddend $\mathbf{v} = ( v_x, v_y ) $ is a vector variable and $\alpha$ a scalar variable.  \DIFaddbegin \DIFadd{Vector $\mathbf{v}$ holds pixel $\mathbf{p}$  values normalized between $-2$ and $2$ and combined with the function $ w(z) $ it produces zero for every combination of $(i,j)$ except for the $4$ adjacent spatial bins.  }\DIFaddend On the other hand, $r$ is an integer that can vary freely \DIFdelbegin \DIFdel{between $ \left[ -1,1 \right]$ which allows the argument }\DIFdelend \DIFaddbegin \DIFadd{in the set $\{ -1, 0, 1 \} $ and }\DIFaddend $\alpha$ \DIFdelbegin \DIFdel{to be unconstrained in terms of its values in radians.  The interpolating function $w(\cdot)$ is defined as $ w(z) = \max(0,|z|-1)$}\DIFdelend \DIFaddbegin \DIFadd{is the difference between the gradient orientation angle and the angle bin center in radians}\DIFaddend .  \DIFaddbegin \DIFadd{Equation~\ref{eq:wang} is used with $ w(z) $ to obtain zero for every angle bin center except for the $2$ adjacent angular bins.
}\DIFaddend 
