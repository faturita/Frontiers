...
%DIF DEL frontiers.old.tex   Tue Mar 12 16:55:16 2019
%DIF ADD frontiers.tex       Thu Mar 14 09:03:57 2019
...
\section{}
Word Count: \DIFdelbegin \DIFdel{5132 }\DIFdelend \DIFaddbegin \DIFadd{5552 }\DIFaddend \\
...
\noindent where $\gamma > 0$ is an input parameter of the algorithm and  it is related to the image scale. In addition, $ x^l(n,c) $ is the point-to-point averaged multichannel EEG signal for the sample point $n$ and for channel $c$. Lastly, $$\bar{x}^l(c) =\frac{1}{n_{max}}\sum_{n=1}^{n_{max}}x^l(n,c)$$ and $$ \hat{\sigma}^l(c) =  \DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \bigg \{  \DIFaddend \frac{1}{n_{max}-1}\sum_{n=1}^{n_{max}} \DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin { \left[  \DIFaddend x^l(n,c)-\bar{x}^l(c)  \DIFdelbegin \DIFdel{)}\DIFdelend \DIFaddbegin \right]  }\DIFaddend ^2 \DIFdelbegin \DIFdel{)}\DIFdelend \DIFaddbegin \bigg \} \DIFaddend ^{\frac{1}{2}}$$ are the mean and estimated standard deviation of $x^l(n,c), 1 \leq n \leq n_{max}$, for each channel $c$.

Consequently, a binary image $I^{(l,c)}$ is constructed according to


\begin{equation}
I^{(l,c)}(z_1,z_2) = \left\{ \DIFdelbegin %DIFDELCMD < \begin{array}{rl}
%DIFDELCMD < 255 & \text{if} \,  z_1 = \gamma \  n; \! z_2 = \tilde{x}^l(n,c) + z^l(c) \\
...
\DIFaddbegin \DIFadd{The work of Edelman, Intrator and Poggio~\mbox{%DIFAUXCMD
\citep{cogprints561} }%DIFAUXCMD
on how the visual cortex sense features was the inspiration to the development of an algorithm to identify and decode salient local information from image regions.  The Scale Invariant Feature Transform (SIFT) is a Computer Vision method proposed by~\mbox{%DIFAUXCMD
\cite{Lowe2004} }%DIFAUXCMD
which is composed of two parts, the SIFT Detector and the SIFT Descriptor.  The former is the procedure to identify relevant areas of an image whereas the latter is the procedure to describe and characterize a region of an image (i.e. patch) calculating an histogram of the gradient orientations.  In order to characterize EEG signal waveforms, this work proposes an alternative to the SIFT Descriptor, the Histogram of Gradient Orientations algorithm.  We stripped away all the details which are not used and refined the procedure to enable the effective capturing of the waveform shape.
}
...
A local representation of the signal shape within the patch can be described by obtaining the gradient orientations on each of the $16$ blocks $B_{i,j}$  with $ 0 \leq i,j \leq 3$ and creating a histogram of gradients.  \DIFdelbegin \DIFdel{This technique is based on Lowe's SIFT~\mbox{%DIFAUXCMD
\citep{Lowe2004} }%DIFAUXCMD
method, and it is biomimetically inspired in how the visual cortex detects shapes by analyzing orientations~\mbox{%DIFAUXCMD
\citep{cogprints561}}%DIFAUXCMD
.   }\DIFdelend In order to calculate the histogram, the interval $[0,360]$ of possible angles is divided in $8$ bins, each one of $45$ degrees.
...
\begin{equation}
 h(\theta,i,j) = 3 s \sum_{\mathbf{p} \in I^{(l,c)}} w_\mathrm{ang}(\angle J(\mathbf{p}) - \theta)\, w_{ij}\left(\frac{\mathbf{p} - \mathbf{p_k}}{3 s}\right)\, \DIFdelbegin \DIFdel{|}\DIFdelend \DIFaddbegin \left\lVert \DIFaddend J(\mathbf{p}) \DIFdelbegin \DIFdel{|
}\DIFdelend \DIFaddbegin \right\rVert 
\DIFaddend \label{eq:histogram}
\end{equation}

\noindent  where $\mathbf{p}$ is a pixel from the image $I^{(l,c)}$,  $\theta$ is the angle bin with $ \theta \in \{0, 45, 90, 135, 180, 225, 270, 315\} $,  \DIFdelbegin \DIFdel{$ |J(\mathbf{p})| $ }\DIFdelend \DIFaddbegin \DIFadd{$ \left\lVert J(\mathbf{p}) \right\rVert $ }\DIFaddend is the norm of the gradient vector in the pixel $\mathbf{p}$ and it is computed using finite differences and $\angle J(\mathbf{p}) $ is the angle of the gradient vector.  
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin 

\DIFadd{The contribution of each gradient vector to the histogram calculated by Equation~\ref{eq:histogram} is balanced by a trilinear interpolation.  The }\DIFaddend scalar $ w_\mathrm{ang}(\cdot) $  and vector $ w_{ij}(\cdot) $ functions are linear interpolations used by~\cite{Lowe2004} and \cite{Vedaldi2010} to provide a weighting contribution to \DIFaddbegin \DIFadd{the }\DIFaddend eight adjacent bins \DIFaddbegin \DIFadd{in the tridimensional histogram}\DIFaddend .  They are calculated as  
...
\noindent where $x_i$ and $y_i$ are the spatial bin centers located in $ x_i,y_j \in \{-\frac{3}{2},-\frac{1}{2},\frac{1}{2},\frac{3}{2}\} $ \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and the interpolating function $w(\cdot)$ is defined as $ w(z) = \max(0,|z|-1)$. The function parameter }\DIFaddend $\mathbf{v} = ( v_x, v_y ) $ is a vector variable and $\alpha$ a scalar variable.  \DIFaddbegin \DIFadd{Vector $\mathbf{v}$ holds pixel coordinates $( v_x, v_y ) $ normalized between $-2$ and $2$ and combined with the function $ w(z) $ it produces zero for every combination of $(i,j)$ except for the $4$ adjacent spatial bins.  }\DIFaddend On the other hand, $r$ is an integer that can vary freely \DIFdelbegin \DIFdel{between $ \left[ -1,1 \right]$ which allows the argument }\DIFdelend \DIFaddbegin \DIFadd{in the set $\{ -1, 0, 1 \} $ and }\DIFaddend $\alpha$ \DIFdelbegin \DIFdel{to be unconstrained in terms of its values in radians.  The interpolating function $w(\cdot)$ is defined as $ w(z) = \max(0,|z|-1)$}\DIFdelend \DIFaddbegin \DIFadd{is the difference between the gradient orientation angle and the angle bin center in radians}\DIFaddend .  \DIFaddbegin \DIFadd{By following this procedure, summands on Equation~\ref{eq:wang} are nullified except for the $2$ adjacent angular bins.
}\DIFaddend 

These binning functions conform \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend trilinear interpolation that has a combined effect of sharing the contribution of each oriented gradient between their eight adjacent bins in a tridimensional cube in the histogram space, and zero everywhere else\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep{Mortensen2005}}%DIFAUXCMD
}\DIFaddend .
...
In order to identify the selected letter, the template set $T^{bpc}$ is used as a database.  Thus, new \DIFdelbegin \DIFdel{descriptors }\DIFdelend \DIFaddbegin \DIFadd{unclassified descriptors $\mathbf{q}^{(l,bpc)} $  }\DIFaddend are computed and they are compared against the descriptors belonging to the calibration template set $T^{bpc}$.

\DIFaddbegin \DIFadd{The Naive Bayes Nearest Neighbor (k-NBNN)~\mbox{%DIFAUXCMD
\citep{Boiman2008} }%DIFAUXCMD
is a discriminative~\mbox{%DIFAUXCMD
\citep{WolpawJonathanR2012} }%DIFAUXCMD
semi-supervised classification algorithm that allows the categorization of an image to one class by comparing the set of extracted descriptors to those which are more similar from template dictionaries.  This work proposes an adapted version to obtain a unary classification scheme to identify the selected letter in the P300-Based BCI Speller, based on the features provided by the calculated descriptors.  
}
...
\begin{equation}
\hat{row} = \arg \min_{l \in \{1,\dots,6\}} \sum\DIFdelbegin \DIFdel{_{q \in N_T(\mathbf{d}^{(l,bpc)})}^{} }\DIFdelend \DIFaddbegin \DIFadd{_{h=1}^{k}  }\DIFaddend {\left\lVert \DIFdelbegin \DIFdel{q }\DIFdelend \DIFaddbegin \DIFadd{\mathbf{q}^{(l,bpc)} }\DIFaddend -  \mathbf{d}\DIFdelbegin \DIFdel{^{(l,bpc)} }\DIFdelend \DIFaddbegin \DIFadd{_{h}^{(bpc)} }\DIFaddend \right\rVert}  ^{2}
...
\begin{equation}
\hat{col} = \arg \min_{l \in \{7,\dots,12\}} \sum\DIFdelbegin \DIFdel{_{q \in N_T(\mathbf{d}^{(l,bpc)})}^{} }\DIFdelend \DIFaddbegin \DIFadd{_{h=1}^{k}  }\DIFaddend {\left\lVert \DIFdelbegin \DIFdel{q }\DIFdelend \DIFaddbegin \DIFadd{\mathbf{q}^{(l,bpc)} }\DIFaddend -  \mathbf{d}\DIFdelbegin \DIFdel{^{(l,bpc)} }\DIFdelend \DIFaddbegin \DIFadd{_{h}^{(bpc)} }\DIFaddend \right\rVert}  ^{2}
\label{eq:multiclassificationcol}
\end{equation}

\noindent \DIFdelbegin \DIFdel{where $N_T(\mathbf{d}^{(l,bpc)})$  is definedas $N_T(\mathbf{d}^{(l,bpc)}) = \{\mathbf{d} \in T^{bpc} / $  is the k-nearest neighbor of $ \mathbf{d}^{(l,bpc)} \}$ }\DIFdelend \DIFaddbegin \DIFadd{with $\mathbf{d}_{h}^{(bpc)}$ belonging to the set $N_T( \mathbf{q}^{(l,bpc)}  )$, which is defined, }\DIFaddend for the best performing channel\DIFaddbegin \DIFadd{,  as $N_T(\mathbf{q}^{(l,bpc)} ) = \{ \mathbf{d}_{h}^{(bpc)} \in T^{bpc} /  \mathbf{d}^{(bpc)} $  is the $k$-nearest neighbor of $ \mathbf{q}^{(l,bpc)} \}$}\DIFaddend . This set is obtained by sorting all the elements in $T^{bpc}$ based on distances between them and \DIFdelbegin \DIFdel{$\mathbf{d}^{(l,bpc)}$}\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{q}^{(l,bpc)}$}\DIFaddend , choosing the $k$ with smaller values, with $k$ a parameter of the algorithm.  
\DIFdelbegin \DIFdel{This procedure is based on the k-NBNN  algorithm~\mbox{%DIFAUXCMD
\citep{Boiman2008}}%DIFAUXCMD
.
}\DIFdelend 
...
The algorithm is implemented \DIFdelbegin \DIFdel{using  VLFeat~\mbox{%DIFAUXCMD
\citep{Vedaldi2010} }%DIFAUXCMD
Computer Vision libraries on MATLAB V2014a }\DIFdelend \DIFaddbegin \DIFadd{on MATLAB V2017a }\DIFaddend (Mathworks Inc., Natick, MA, USA). \DIFaddbegin \DIFadd{The algorithm described in~\ref{SIFT} is implemented on a modified version of the  VLFeat~\mbox{%DIFAUXCMD
\citep{Vedaldi2010} }%DIFAUXCMD
Computer Vision library.   }\DIFaddend Furthermore, in order to enhance the impact of \DIFdelbegin \DIFdel{our }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend paper and for a sake of reproducibility, the code of the \DIFdelbegin \DIFdel{algorithm }\DIFdelend \DIFaddbegin \DIFadd{entire algorithm, including the modified VLFeat library, }\DIFaddend has been made available at: https://bitbucket.org/itba/hist.
...
\DIFdelbegin \DIFdel{Lastly}\DIFdelend \DIFaddbegin \DIFadd{Moreover}\DIFaddend , the number of channels $C$ is equal to $8$ for both datasets, and the number of intensification sequences $k_a$ is fixed to $10$.  The parameter $k$ used to construct the set \DIFdelbegin \DIFdel{$N_T(\mathbf{d}^{(l,c)})$ }\DIFdelend \DIFaddbegin \DIFadd{$N_T(\mathbf{q}^{(l,c)})$ }\DIFaddend is assigned to $k=7$, which was found empirically to achieve better results.  In addition, the norm used on  Equations \ref{eq:multiclassificationrow} and \ref{eq:multiclassificationcol} is the cosine norm, and descriptors are normalized to $ \left[ -1, 1 \right] $.

\DIFaddbegin \DIFadd{Lastly, in order to assess the validity of the Histogram of Gradient Orientations (HIST) method, the character recognition rate for both datasets is evaluated replicating the methodology proposed by the ALS dataset's publisher, since authors \mbox{%DIFAUXCMD
\cite{Riccio2013} }%DIFAUXCMD
did not report the Character Recognition Rate obtained for this dataset. Frequency filtering, data segmentation and artifact rejection is conducted according to Section~\ref{Pipeline} yielding 16 x 8 samples per epoch.  A multichannel feature consists of time points vector~\mbox{%DIFAUXCMD
\citep{Lotte2018}}%DIFAUXCMD
, formed by concatenating all the channels~\mbox{%DIFAUXCMD
\citep{Krusienski2006}}%DIFAUXCMD
. A single-channel variant consists of using time points from a single electrode and performing the analysis on a channel-by-channel basis.  Three classification schemes are considered as well. A multichannel version of the  Stepwise Linear Discriminant Analysis (SWLDA) classification algorithm. SWLDA is the methodology proposed by the ALS dataset's publisher. Additionally, a single-channel and a multichannel variant of a linear kernel Support Vector Machine (SVM)~\mbox{%DIFAUXCMD
\citep{Scholkopf2001} }%DIFAUXCMD
classifier are utilized.   SVM has been successfully used in several BCI Competitions~\mbox{%DIFAUXCMD
\citep{Rakotomamonjy2008}}%DIFAUXCMD
.
}
...
In Table~\ref{tab:resultsown} the results obtained for 8 healthy subjects are shown.  It can be observed that the performance is above chance level. It \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend verified that HIST method has an improved performance at letter identification than SVM that process the signals on a channel by channel strategy (Wilcoxon signed-rank test, $p =  0.004$ for both datasets).
...
%DIFDELCMD < %%%
\DIFdelend Tables~\ref{tab:resultsalsswlda} and~\ref{tab:resultsownswlda} are presented in order to compare the performance of the HIST method versus  \DIFdelbegin \DIFdel{a multichannel version of the Stepwise Linear Discriminant Analysis (SWLDA ) }\DIFdelend \DIFaddbegin \DIFadd{multichannel SWLDA }\DIFaddend and SVM classification algorithms for both datasets.   \DIFdelbegin \DIFdel{The feature was formed by concatenating all the channels~\mbox{%DIFAUXCMD
\citep{Krusienski2006}}%DIFAUXCMD
.  SWLDA is the methodology proposed by the ALS dataset's publisher. Since authors \mbox{%DIFAUXCMD
\cite{Riccio2013} }%DIFAUXCMD
did not report the Character Recognition Rate obtained for this dataset, we replicate their procedure and include the performance obtained with the SWLDA algorithm at letter identification.  It was }\DIFdelend \DIFaddbegin \DIFadd{It is }\DIFaddend verified for the dataset of ALS patients that it has similar performance  against other methods like SWLDA or SVM, which use a multichannel feature (Quade test with $p=0.55$) whereas for the dataset of healthy subjects significant differences \DIFdelbegin \DIFdel{were }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend found (Quade test with $p=0.02$) where only the HIST method \DIFdelbegin \DIFdel{achieved }\DIFdelend \DIFaddbegin \DIFadd{achieves }\DIFaddend a different performance than SVM (with multiple comparisons, significant difference of level $0.05$).
...
\DIFadd{To further extend the capabilities of this method, it would be desirable to implement a multichannel version. The straightforward extension of concatenating the obtained descriptors results in high dimensional feature vector, while other variants that merge descriptors per channel may diminish the mutual information between different channels.  Hitherto variants using color versions of SIFT~\mbox{%DIFAUXCMD
\citep{VanDeSande2010}}%DIFAUXCMD
, where different color bands are mapped to electrode channels, have been explored without substantial success}\DIFaddend .
