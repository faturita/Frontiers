...
%DIF DEL frontiers.review.tex   Wed Aug  8 21:56:14 2018
...
\section{}
Word Count: \DIFdelbegin \DIFdel{4952 }\DIFdelend \DIFaddbegin \DIFadd{5132 }\DIFaddend \\
...
Consequently, \DIFdelbegin \DIFdel{for a pixel $(z_1,z_2)$, the }\DIFdelend \DIFaddbegin \DIFadd{a binary }\DIFaddend image $I^{(l,c)}$ is constructed according to
...
Table~\ref{tab:resultsals} shows the results of applying the \DIFdelbegin \DIFdel{proposed }\DIFdelend \DIFaddbegin \DIFadd{Histogram of Gradient Orientations (HIST) }\DIFaddend algorithm to the subjects of the public dataset of ALS patients. The percentage of correctly spelled letters is calculated while performing an offline BCI Simulation.  From the seven words for each subject, the first three are used for calibration, and the remaining four are used for testing.  The best performing channel  $bpc$ is informed as well. The target ratio is $1:36$; hence \DIFaddbegin \DIFadd{theoretical }\DIFaddend chance level is $2.8\%$. It can be observed that the best performance of the letter identification method is reached in a dissimilar channel depending on the subject being studied.  \DIFdelbegin \DIFdel{This table shows }\DIFdelend \DIFaddbegin \DIFadd{Table~\ref{tab:resultsals} and~\ref{tab:resultsown} show }\DIFaddend for comparison the obtained performance rates using single-channel signals with the \DIFdelbegin \DIFdel{SVM}\DIFdelend \DIFaddbegin \DIFadd{Support Vector Machine (SVM)}\DIFaddend ~\citep{Scholkopf2001} classifier.  This method is configured to use a linear kernel.  The best performing channel, where the best letter identification rate was achieved, is also depicted.
...
In Table~\ref{tab:resultsown} the results obtained for 8 healthy subjects are shown.  It can be observed that the performance is above chance level\DIFdelbegin \DIFdel{and higher than those achieved by the other method}\DIFdelend \DIFaddbegin \DIFadd{. It was verified that HIST method has an improved performance at letter identification than SVM that process the signals on a channel by channel strategy (Wilcoxon signed-rank test, $p =  0.004$ for both datasets)}\DIFaddend .
...
Tables~\ref{tab:resultsalsswlda} and~\ref{tab:resultsownswlda} are presented in order to compare the performance of the \DIFdelbegin \DIFdel{Histogram of Gradient Orientations (HIST ) }\DIFdelend \DIFaddbegin \DIFadd{HIST }\DIFaddend method versus a multichannel version of the \DIFdelbegin \DIFdel{SWLDA }\DIFdelend \DIFaddbegin \DIFadd{Stepwise Linear Discriminant Analysis (SWLDA) }\DIFaddend and SVM classification algorithms for both datasets.  The feature was formed by concatenating all the channels~\citep{Krusienski2006}.  SWLDA is the methodology proposed by the ALS dataset's publisher. Since authors \cite{Riccio2013} did not report the Character Recognition Rate obtained for this dataset, we replicate their procedure and include the performance obtained with the SWLDA algorithm at letter identification.  It was verified \DIFdelbegin \DIFdel{that HIST method has an improved performance at letter identification than other methods that process the signals on a channel by channel strategy, and it even has a comparable }\DIFdelend \DIFaddbegin \DIFadd{for the dataset of ALS patients that it has similar }\DIFaddend performance  against other methods like SWLDA or SVM, which \DIFdelbegin \DIFdel{uses }\DIFdelend \DIFaddbegin \DIFadd{use }\DIFaddend a multichannel feature \DIFaddbegin \DIFadd{(Quade test with $p=0.55$) whereas for the dataset of healthy subjects significant differences were found (Quade test with $p=0.02$) where only the HIST method achieved a different performance than SVM (with multiple comparisons, significant difference of level $0.05$)}\DIFaddend .
...
\begin{table}[htb]
\caption{Character recognition rates \DIFaddbeginFL \DIFaddFL{and the best performing channel $bpc$ }\DIFaddendFL for the public dataset of ALS patients using the Histogram of Gradient (HIST) \DIFdelbeginFL \DIFdelFL{calculated from plots of single channels with the best performing channel $bpc$ informed as well}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(repeated here for comparison purposes)}\DIFaddendFL . Performance rates obtained by SWLDA and SVM classification algorithms with a multichannel concatenated feature.}
\centering
%% \tablesize{} %% You can specify the fontsize here, e.g.  \tablesize{\footnotesize}. If commented out \small will be used.
...
\begin{table}[htb]
\caption{Character recognition rates \DIFaddbeginFL \DIFaddFL{and the best performing channel $bpc$ }\DIFaddendFL for the own dataset of healthy subjects using the Histogram of Gradient (HIST) \DIFdelbeginFL \DIFdelFL{calculated from plots of single channels with the best performing channel $bpc$ informed as well}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(repeated here for comparison purposes)}\DIFaddendFL .   Performance rates obtained by SWLDA and SVM classification algorithms with a multichannel concatenated feature.}
\centering
%% \tablesize{} %% You can specify the fontsize here, e.g.  \tablesize{\footnotesize}. If commented out \small will be used.
\begin{tabular}{c|cc|c|c}
\toprule
